...
Legacy device and service state lives in `~/.config/yokai/config.json`. Coordinated deployment state is stored separately in the versioned `~/.config/yokai/deployments.json`; older config writers therefore cannot silently discard deployment groups. Coordinated Qwen launch signing uses a separate owner-only `~/.config/yokai/coordinator-signing-key.json`; it is not an agent bearer token, is never copied to a GPU node, and must be preserved with the coordinator configuration. Bootstrap writes the selected stable `devices[].id` to the target agent's `agent.json` as `device_id` and writes only the coordinator's Ed25519 public key as the distinct `coordinator_public_key`; changing a device ID requires bootstrapping that agent again before a coordinated Qwen launch.
...
**96 serving configs (92 vLLM plus four SGLang) across 90 models from 26 publishers** (as of this commit). Entries are grouped by publisher and live in `internal/bkc/catalog_*.go`:
...
| `nvidia` | 15 | 18 | [`catalog_nvidia.go`](internal/bkc/catalog_nvidia.go), [`catalog_google.go`](internal/bkc/catalog_google.go), [`catalog_llama.go`](internal/bkc/catalog_llama.go), [`catalog_moonshotai.go`](internal/bkc/catalog_moonshotai.go), [`catalog_qwen.go`](internal/bkc/catalog_qwen.go), [`catalog_qwen38_dual.go`](internal/bkc/catalog_qwen38_dual.go) |
...
- **Qwen3.8 Flash Next dual DGX Spark** — `qwen3-8-flash-next-nvfp4-dual-gb10` pins the MiaAI-Lab dual-Spark source, NVIDIA model revision, and arm64 vLLM image digest for two one-GPU GB10 nodes. It launches rank 1 headless before the rank-0 API, with TP2, expert parallelism, three-token MTP, FP8 KV cache, and native vLLM readiness. Per-node fabric IP/interface/HCA/GID values are deployment inputs and preflight proves the selected GID encodes that interface's fabric IP. The snapshot must contain exactly the pinned upstream object set and every object is content-hashed. Each agent validates only the eleven expected weight blobs before best-effort clean page-cache release, serializes local Yokai GPU admission, and repeats the zero-tenant observation immediately before `docker run`; unrelated external processes remain an OS-level race without NVIDIA or hardware isolation. The source-required PLE, MXFP8, MTP, FP8-KV, and checkpoint adaptations remain exact-source/exact-output-hash runtime patches with no shell or writable host snapshot.
...
Multi-device recipes carry typed cluster metadata and cannot be submitted to the legacy `POST /deploy` API. Submit explicit `head` and `worker` bindings to `POST /deployments` instead. Both device IDs must be distinct, both fabric addresses must be valid non-loopback IPs, and the head binding must include a separate explicit client/monitor IP (Tailnet or another routable IP) and the recipe service port. Unspecified and loopback service binds are rejected. Both online agents must advertise the recipe's required capabilities and expose exactly one GB10 GPU before Yokai writes a journal or mutates Docker.

Yokai stages both pinned images, stops only explicitly selected observed old container IDs, launches each recipe in its validated order, waits for both members, and promotes only after the rank-0 API/model/native-metrics test passes. Before removing candidates after a failure, rollback best-effort captures and persists bounded, sanitized worker and non-Qwen diagnostic tails; Qwen3.8 rank 0 is never tailed because its process retains the API key and caller-supplied redaction cannot make that safe. It then removes candidates in reverse launch order, restarts selected old IDs, and persists the rollback result. Yokai never auto-adopts external containers; unselected external containers remain visible and testable but immutable. Generic container log streaming remains available for ordinary non-grouped containers but rejects every deployment-group member and every Qwen3.8 BKC head even if its deployment label is missing before invoking `docker logs`; ungrouped Qwen3.8 BKC launches are rejected before Docker mutation. Existing managed-only container and metrics views are unchanged, and stopped services and deployment groups remain visible.

Qwen3.8 candidate creation also requires a short-lived Ed25519 authorization signed by the local coordinator. The signed claims bind the exact target `binding.DeviceID`, deployment ID, generation, deterministic candidate name, role, BKC ID, model/source/image provenance, expiration, and a digest of the complete non-secret launch request. Each agent compares that signed target to its bootstrap-provisioned local `device_id` before consuming the authorization or running any Docker/image command, so separate agent replay stores cannot turn a token for one node into a token for another; hostnames, IPs, bearer authentication, and caller-supplied labels do not establish target identity. The agent verifies with the distinct bootstrap-provisioned coordinator public key and durably consumes the random authorization ID before mutation, keeping the token single-use across requests and agent restarts. The request-time model API key is excluded from the claims and launch digest. The Qwen recipe requires an agent capability advertised only while local identity, verifier, and replay state are all available; authenticated system info exposes only the verifier's non-secret SHA-256 fingerprint, and daemon preflight rejects a reported identity or fingerprint that differs from the selected binding and active coordinator signing key. Existing agents must be bootstrapped again after upgrading to receive both `device_id` and `coordinator_public_key`; otherwise Qwen fails preflight before journaling or Docker mutation while non-Qwen and legacy behavior remains unchanged. If the configured device ID changes or `coordinator-signing-key.json` is lost or intentionally rotated, bootstrap every participating agent again before launching Qwen.

The API key is required and request-scoped in Yokai. With the CLI, pass the name of an environment variable via `--api-key-env` on `create`, `test`, and `start`; OpenTUI uses a masked transient field. Starting a stopped group restarts its existing containers, so the operator must supply the original launch key; start cannot rotate it, and a new key requires a new deployment. Yokai sends a single structured `--api-key=<value>` token only to rank 0 and omits it from the BKC, `deployments.json`, deployment responses, inventory, and Yokai logs. SGLang has no documented secret-fd input, so Docker `Config.Cmd` and rank 0's process argv necessarily retain the key while that container exists. A supplied `--local-model-path` must be an explicit clean absolute path that exists at the same location on both devices. The GLM recipe mounts that directory read-only at `/models/yokai-deployment`; Qwen3.8 validates and mounts the surrounding standard Hugging Face repository cache so its content-addressed snapshot links remain valid.
...
For Qwen3.8, the pristine standard Hugging Face cache snapshot is required at the same absolute path on both nodes; its repository directory and final revision component must match the pinned model. Preflight verifies the exact 25-object set and streams every Git or LFS content digest, launch revalidates all eleven expected weight blobs before page-cache release, and every restart repeats the complete manifest/object/digest/index verification before Docker mutation, so these phases need time and disk bandwidth proportional to the complete checkpoint. Only rank 0 reserves and validates the API service port; the headless worker retains fabric, NCCL, rendezvous, snapshot, and GPU safety checks without treating an occupied local 8888 as its listener. Each agent also requires working `nvidia-smi`, readable `/proc/<pid>/cgroup` entries for reported compute processes, and Linux `POSIX_FADV_DONTNEED`; only the advice call is best-effort, while unsafe weight identity blocks launch. The agent-local admission lock serializes Yokai checks and launches but cannot prevent a new external GPU process after the final observation without NVIDIA or hardware isolation. The interface, HCA, and GID values below are illustrative per-node inputs and must match agent preflight on each physical Spark:

```bash
export YOKAI_CANARY_API_KEY='replace-me'
yokai deployments create \
  --bkc qwen3-8-flash-next-nvfp4-dual-gb10 \
  --idempotency-key qwen38-dual-canary-001 \
  --head-device spark-a --head-fabric 10.20.0.10 \
  --head-fabric-interface enP2p1s0f0np0 --head-fabric-hca roceP2p1s0f0 --head-fabric-gid-index 3 \
  --head-service-address 100.100.100.100 --head-service-port 8888 \
  --worker-device spark-b --worker-fabric 10.20.0.11 \
  --worker-fabric-interface enP2p1s0f0np0 --worker-fabric-hca roceP2p1s0f0 --worker-fabric-gid-index 3 \
  --api-key-env YOKAI_CANARY_API_KEY \
  --local-model-path /srv/hf/hub/models--nvidia--Qwen3.8-Flash-Next-NVFP4/snapshots/fc694b54fb0174e0913e6adf86691ef85a4ead47
```

The Qwen3.8 agents also need HTTPS access to GitHub's raw-content endpoint on first launch, or an operator-prepopulated exact-hash runtime asset cache. See [the multi-device architecture](architecture/08-multi-device-deployments.md) for the immutable revisions, preflight contract, patch strategy, and readiness gate.
